...
Our test suite covers endpoints, core services, and logic using `pytest` and `pytest-cov`.

### Test Summary
Based on the latest local test run across the full pipeline (including live models):
* **79 tests passed**
* **119 warnings** generated (mostly related to dependency deprecations or Hugging Face internals).
* **Execution Time:** ~12m 44s (764.95s).
* **Code Coverage:** **95%** across the backend (800 statements evaluated, 39 missed).
...
All tests including real model inference (this matches the test summary above):
...
The API is instrumented with OpenTelemetry and exports traces, metrics, and logs directly to [Axiom](https://axiom.co) via OTLP/HTTP. No Docker or OpenTelemetry Collector is required for local development.

### Setup Monitoring

Add the following to `deployment/.env`:
...
Our monitoring implementation focuses on three primary metrics across system layers:
...
Additional signals tracked: `data.feedback.vote` (thumbs up/down), `http.server.request.duration`, `server.process.uptime`, and `http.server.active_requests`.
...
MPL dashboard queries: [deployment/monitoring/axiom-dashboard-queries.txt](deployment/monitoring/axiom-dashboard-queries.txt)
